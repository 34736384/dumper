//Netvar system taken from CSGOSimple
#include "netvars.h"

#include <fstream>
#include <utility>

std::unique_ptr<NetvarSys> netvars = std::make_unique<NetvarSys>();
...
uint32_t NetvarSys::GetOffset(const std::string& tableName, std::string propName)
{
	auto result = 0u;
	for (const auto& table : database) {
